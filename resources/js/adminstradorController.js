app.controller('administradorController', function ($scope, $http, APP_DEFAULT_URL, $window) {
	...
	$scope.getData = function () {
		...
		$http.get("https://api-sepomex.hckdrk.mx/query/info_cp/" + $scope.enlaceData.residence.zipCode + "?token=55d025b2-48cd-46db-8748-6955ac7a580d", {}).then(function (response) {
	...
	$scope.getData2 = function () {
		...
		$scope.editarEnlace.residence.street = null;
		$scope.editarEnlace.residence.municipality = "";
		$scope.editarEnlace.residence.locality = "";
		//datos de localidad por cp
		$http.get("https://api-sepomex.hckdrk.mx/query/info_cp/" + $scope.editarEnlace.residence.zipCode + "?token=55d025b2-48cd-46db-8748-6955ac7a580d", {}).then(function (response) {
			console.log(response.data)
			$scope.domicilioData = response.data;
			$scope.colonias = response.data;
	...
	$scope.EditarEnlace = function (enlace) {
		...
		var codigoPostalEditar = $scope.editarEnlace.residence.zipCode;
	...
	$scope.getLogBook = function () {
		...
		if ($scope.token != null) {
			...
			console.log($scope.sesion);
			if ($scope.sesion.role == "Administrador") {
				$http.get(APP_DEFAULT_URL.url + "admin/logbook", {
					headers: {
